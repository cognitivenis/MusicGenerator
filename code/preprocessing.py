...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 30 20:30:50 2019
...
def generate_dict_time_notes(list_all_midi, batch_song = 16, start_index=0, fs=30, use_tqdm=True):
    ...
    dict_time_notes = {}
    process_tqdm_midi = tqdm(range(start_index, min(start_index + batch_song, len(list_all_midi))))
...
def get_sampled_midi(composer='', size=100):
    if (composer != ''):
        midi_csv = pd.read_csv('../maestro-v1.0.0/maestro-v1.0.0.csv')
        midi_csv = midi_csv[midi_csv['canonical_composer'] == composer]
        midi_csv.reset_index(inplace=True)
        midi_names = midi_csv['midi_filename']

        sampled_midi = glob.glob(os.path.join('../maestro-v1.0.0/', midi_names[0]))
        for name in midi_names:
            sampled_midi += glob.glob(os.path.join('../maestro-v1.0.0/', name))
        sampled_midi = sampled_midi[1:]

    else:
        list_all_midi = glob.glob('../maestro-v1.0.0/**/*.midi')
